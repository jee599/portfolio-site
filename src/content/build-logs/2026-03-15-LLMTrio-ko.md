---
title: "Claude Code 멀티에이전트 오케스트레이터: 3개 LLM 병렬 실행, 86세션의 삽질 기록"
project: "LLMTrio"
date: 2026-03-15
lang: ko
tags: [LLM, orchestration, multi-agent, claude-code, automation]
description: "Claude Code 멀티에이전트로 3개 LLM을 병렬 실행하는 오케스트레이터를 만들었다. 86세션 동안 발견한 보안 이슈, 컨텍스트 손실, 프롬프트 설계 패턴을 기록한다."
---

멀티에이전트 LLM 오케스트레이션은 아이디어는 단순하다. Claude, Codex, Gemini를 동시에 돌려서 태스크를 모델 강점에 맞게 자동 분배하면 된다. 그런데 86세션을 돌려보니 같은 보안 버그가 3번 발견되고, TypeScript 설정은 매번 무시됐고, 크레딧은 하루 만에 바닥났다.

**TL;DR**: Claude Code 멀티에이전트 워크플로우에서 컨텍스트는 명시적으로 주입해야 하고, 발견된 버그는 즉시 코드에 반영해야 한다. 프롬프트 제약이 강할수록 결과물이 안정된다.

## `npx llmtrio` 한 줄로 3개 LLM이 병렬 실행된다
...
## 컨텍스트 혼동: "claude book"을 수십 번 물어본 이유
...
- [Claude Code로 XRP 트레이딩 봇 만들기: 14 세션, 961번의 도구 호출](/posts/2026-03-15-trading-bot-ko)
