---
title: "Claude Code 훅으로 AI 책 쓰기 자동화 — PostToolUse가 25번 루프한 이야기"
project: "claudebook"
date: 2026-03-15
lang: ko
tags: [claude-code, hooks, automation, crawling]
description: "Claude Code 훅(PostToolUse)으로 AI 책 쓰기 시장 조사를 자동화했다. 에이전트가 25번 루프한 원인과 해결책, 34개 문서 크롤링 과정을 기록한다."
---

Claude Code 훅 하나를 잘못 설정하면 에이전트가 25번 루프한다. Claude Code로 Claude Code에 대한 책을 쓰면서 겪은 일이다. 26개 세션, 147번의 tool call 중 실제로 생산적이었던 건 딱 3개 세션이었다.

**TL;DR**: PostToolUse 훅은 에이전트 자체의 도구 사용도 트리거한다. 루프 방지 조건 없이 훅을 설정하면 같은 작업이 수십 번 반복된다. AI 책 쓰기 자동화는 가능하지만, 훅 설계가 먼저다.

## PostToolUse 훅으로 시장 조사 에이전트를 자동화했다
...
## WebSearch가 차단되자 에이전트가 스스로 WebFetch로 전환했다
