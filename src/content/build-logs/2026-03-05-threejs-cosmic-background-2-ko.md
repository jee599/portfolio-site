...
Claude한테 물었더니, CSS reset의 `* { max-width: 100% }` 룰이 canvas를 죽이고 있다는 걸 찾아냈다.
...
canvas를 reset에서 제외하는 것으로 첫 번째 픽스 완료.
...
그런데 이번엔 `body` 배경색이 불투명 navy(`#0a0b2a`)로 전부 덮고 있었다.
...
<tbody>
<tr><td class="label">CSS 배경 코드</td><td class="before">51줄</td><td class="after">0줄 (Three.js로 이관)</td></tr>
<tr><td class="label">커밋 수</td><td class="before">—</td><td class="after">5개 (디버깅 4 + 설정 1)</td></tr>
